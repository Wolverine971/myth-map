...
<div class="flex items-center gap-3 {centered ? 'justify-center' : ''}">
	...
	{:else if type === 'dots'}
		<div class="flex space-x-1">
			{#each Array(3) as _, i}
				<div
					class="animate-bounce {sizeClasses[size] === 'h-4 w-4'
						? 'h-2 w-2'
						: sizeClasses[size] === 'h-8 w-8'
							? 'h-3 w-3'
							: 'h-4 w-4'} rounded-full bg-current {colorClasses[color]}"
	...
	{:else if type === 'pulse'}
		<div
			class="animate-pulse {sizeClasses[size]} rounded-full bg-current {colorClasses[color]}"
		></div>
	{:else if type === 'bars'}
		<div class="flex space-x-1">
			{#each Array(4) as _, i}
				<div
					class="animate-pulse {sizeClasses[size] === 'h-4 w-4'
						? 'h-4 w-1'
						: sizeClasses[size] === 'h-8 w-8'
							? 'h-8 w-2'
							: 'h-12 w-2'} rounded bg-current {colorClasses[color]}"
