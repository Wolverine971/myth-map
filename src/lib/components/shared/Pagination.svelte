	...
	>
		...
		<div class="flex items-center gap-1">
			...
			{#if visiblePages[0] > 1}
				...
				{#if visiblePages[0] > 2}
					<span class="flex h-9 w-9 items-center justify-center text-gray-500 sm:h-10 sm:w-10"
						>...</span
					>
				{/if}
			{/if}
			...
			{#each visiblePages as page}
				<button
					class="flex h-9 w-9 items-center justify-center rounded-lg border text-sm font-medium transition-colors sm:h-10 sm:w-10 {page ===
					currentPage
