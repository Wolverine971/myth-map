...
<Card
	padding="none"
	class="transition-smooth hover-lift group h-full overflow-hidden hover:shadow-md"
>
	<div class="flex h-full flex-col sm:flex-row">
		...
		<div class="relative h-40 w-full sm:h-auto sm:w-32 md:w-40">
			...
			{#if tags?.[0]?.tags?.name}
				<div class="absolute left-2 top-2">
					<Badge
						color="primary"
						class="bg-white/90 text-xs font-semibold text-primary-700 shadow-sm"
					>
						{tags[0].tags.name}
					</Badge>
				</div>
		...
		<div class="flex flex-1 flex-col p-4">
			...
			<div class="mb-2">
				<h3
					class="line-clamp-1 text-base font-bold text-gray-900 group-hover:text-primary-700 sm:text-lg"
				>
					{displayName}
				</h3>
